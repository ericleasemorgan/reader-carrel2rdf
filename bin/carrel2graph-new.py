...
if not authors.empty :
	...
	for _, author in authors.iterrows() : seen[ author[ 'author' ] ] = []
	...
	if not authors.empty :
		...
		for _, author in authors.iterrows() :
			...
			if str( author[ 'id' ] ) == idItem :
				...
				authorNumber = author[ 'qnumber' ]
				if type( authorNumber ) is str : graph.add( ( item, DCTERMS.creator, URIRef( WIKIDATA + authorNumber ) ) )
				else : graph.add( ( item, CARREL.hasAuthor, Literal( author[ 'author' ] ) ) )
				
				# check for author/keyword combination
				#if idItem in keywords[ 'id' ].values :
				
				#	name  = author[ 'author' ]
				#	words = list( keywords.loc[ keywords[ 'id' ] == idItem ][ 'keyword' ] )
				#	for word in words :
					
				#		items = seen[ name ]						
				#		if word not in items :
				#			print( name, 'knows about', word )
				#			items.append( word )
				#			seen[ name ] = items
